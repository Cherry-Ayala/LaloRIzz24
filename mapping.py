...
def scan_area(video_source, model, num_frames=30):
    """
    Perform an initial scan of the area to create a background map
    """
    ...
    object_map = np.zeros((frame_height, frame_width), dtype=np.uint8)
    ...
    for _ in range(num_frames):
        ...
        detections = detect(frame[:, :, ::-1], model)[0]
        
        for detection in detections['det']:
            if detection[5] != 0:  # Exclude person class
                x1, y1, x2, y2 = map(int, detection[:4])
                object_map[y1:y2, x1:x2] = 255
                background[y1:y2, x1:x2] = frame[y1:y2, x1:x2]
    
    cap.release()
    return background, object_map
...
def run_object_detection(video_source, model, device="AUTO"):
    ...
    # Perform initial area scan
    background, object_map = scan_area(video_source, quantized_det_compiled_model)
    ...
    while cap.isOpened():
        ...
        if len(df) > 0:  # Only create heatmap if there's data
            ...
            # Display the background image with detected objects
...
def main():
    st.title("Mime Client follower with Area Scanning")
